...
#LOAD PICKLE FILES
model = pickle.load(open(r'C:\Users\Rohit\Desktop\fake_review_detector-main\data and pickle files\best_model.pkl','rb')) 
...
def text_classification(text):
    if len(text) < 1:
        ...
        with st.spinner("Classification in progress..."):
            ...
            if p == 'True':
                st.success("The review entered is Real.")
            if p == 'False':
                st.error("The review entered is Fake.")

#PAGE FORMATTING AND APPLICATION
def main():
    st.title("Detecting Fake Reviews Using Machine Learning Techniques")
